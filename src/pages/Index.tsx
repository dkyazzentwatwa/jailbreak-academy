...
const Index = () => {
  ...
  const handleExportPDF = () => {
    ...
    // Title
    pdf.setFontSize(16);
    pdf.text('LLM SANITIZATION REPORT', pageWidth / 2, yPosition, {
      align: 'center'
    });
    yPosition += 20;
      ...
      <div className="container mx-auto py-8 px-4 max-w-6xl relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold terminal-green-glow font-mono tracking-wider">LLM_SANITIZER_v0.1</h1>
            <Zap className="h-6 w-6 text-terminal-green animate-pulse" />
          </div>
          <div className="font-mono text-terminal-green text-sm mb-2">
            <span className="terminal-cursor">by little hakr</span>
          </div>
        ...
        <div className="flex items-center justify-center gap-4 mb-6 font-mono">
          ...
          <Switch id="expert-mode" checked={expertMode} onCheckedChange={setExpertMode} className="data-[state=checked]:bg-terminal-green" />
          <Label htmlFor="expert-mode" className="text-terminal-green">[EXPERT_MODE]</Label>
          
          {result && <Button onClick={handleExportPDF} variant="outline" size="sm" className="ml-8 border-terminal-green/50 text-terminal-green hover:bg-terminal-green/20 font-mono">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="h-fit bg-card border-terminal-green/30 terminal-glow">
            ...
            <CardContent className="space-y-4 pt-6">
              <Textarea placeholder=">>> PASTE_AI_OUTPUT_HERE
// WARNING: Treat all input as potentially malicious
// System will analyze for XSS, injections, exploits..." value={input} onChange={e => setInput(e.target.value)} className="min-h-[300px] font-mono text-sm resize-none bg-input border-terminal-green/30 text-terminal-green placeholder:text-muted-foreground focus:border-terminal-green focus:ring-terminal-green/50" />
          ...
          <Card className="h-fit bg-card border-terminal-green/30 terminal-glow">
            ...
            <CardContent className="pt-6">
              {!result ? <div className="text-center py-8 text-muted-foreground border-2 border-dashed border-terminal-green/20 rounded-lg bg-terminal-bg">
                  ...
                  {/* Severity Summary */}
                  <Alert className={`border font-mono ${result.severity === 'high' ? 'border-red-500/50 bg-red-950/20' : result.severity === 'moderate' ? 'border-yellow-500/50 bg-yellow-950/20' : result.severity === 'low' ? 'border-blue-500/50 bg-blue-950/20' : 'border-terminal-green/50 bg-terminal-green/5'}`}>
                    <div className="flex items-center gap-2">
                  ...
                  {/* Issues List */}
                  {(expertMode || result.issues.length > 0) && <IssuesList issues={result.issues} expertMode={expertMode} />}
                    ...
                    <TabsContent value="comparison">
                      <div className="space-y-4">
                        <div>
                          ...
                          <OutputDisplay content={result.originalInput} type="original" />
                        </div>
                        <div>
                  ...
                  {/* Guidance */}
                  {result.guidance && <Alert className="border-terminal-green/30 bg-terminal-green/5">
